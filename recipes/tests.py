...
class UserTestCase(TestCase):
    def setUp(self):
        self.user_a = User.objects.create_user('ammar', password='abc123')
...
class RecipeTestCase(TestCase):
    ...
    def test_user_two_level_relation_reverse(self):
        user=self.user_a
        recipeingredientids = list(user.recipe_set.all().values_list
                                   ('recipeingredient__id', flat=True))
        qs = RecipeIngredient.objects.filter(id__in=recipeingredientids)
        self.assertEqual(qs.count(), 2)
